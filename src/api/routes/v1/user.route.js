...
const { authorize, ADMIN, INSTRUCTOR } = require('../../middlewares/auth');
const {
  listUsers,
  createUser,
  updateUser,
...
router
   ...
   * @apiError (Not Found 404)    NotFound     User does not exist
   */
  .put(authorize(ADMIN), validate(updateUser), controller.update)
  /**
  ...
  .post(authorize(ADMIN), validate(updateUser), controller.update);
